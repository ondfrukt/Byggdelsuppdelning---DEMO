...
class ObjectRelation(db.Model):
    ...
    # FK to relation_types.key — enforces referential integrity on the type string
    relation_type = db.Column(
        db.String(100),
        db.ForeignKey('relation_types.key', ondelete='RESTRICT', name='fk_object_relations_relation_type'),
        nullable=False
    )
    ...
    relation_type_def = db.relationship(
        'RelationType',
        foreign_keys=[relation_type],
        primaryjoin='ObjectRelation.relation_type == RelationType.key',
        uselist=False,
        lazy='select'
    )
    ...
    def to_dict(self, include_objects=True):
        ...
        result = {
            ...
            # Cardinality is now on the type definition, not on each relation instance
            'max_targets_per_source': rt.max_targets_per_source if rt else None,
            'max_sources_per_target': rt.max_sources_per_target if rt else None,
